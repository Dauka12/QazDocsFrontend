...
const CounterpartiesPage = () => {
  ...
  const [newCP, setNewCP] = useState({
    ...
    signatory: { full_name: '', position: '', basis: 'Charter' }
  });
  ...
  return (
    <div className="p-12">
      <div className="flex justify-between items-center mb-12">
        <div>
          <h1 className="text-4xl font-black text-brand-black mb-2">Counterparties</h1>
          <p className="text-brand-black/40 font-bold uppercase tracking-widest text-sm">Manage your clients and partners</p>
        </div>
        <button 
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="bg-brand-eggshell border-b border-brand-black/5">
              <th className="px-8 py-4 text-xs font-black uppercase tracking-widest text-brand-black/40">Name / BIN</th>
              <th className="px-8 py-4 text-xs font-black uppercase tracking-widest text-brand-black/40">Contact</th>
              <th className="px-8 py-4 text-xs font-black uppercase tracking-widest text-brand-black/40">Signatory</th>
                  ...
                  <td className="px-8 py-6">
                    <div className="flex flex-col gap-1">
                      <div className="flex items-center gap-2 text-sm font-medium text-brand-black/60">
                        <Mail size={14} /> {cp.email || 'N/A'}
                      </div>
                      <div className="flex items-center gap-2 text-sm font-medium text-brand-black/60">
                        <Phone size={14} /> {cp.phone || 'N/A'}
                      </div>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    <div className="flex flex-col">
                      <span className="text-sm font-bold text-brand-black">{cp.signatory?.full_name || 'N/A'}</span>
        ...
        {isCreateOpen && (
          <div className="fixed inset-0 bg-brand-black/20 backdrop-blur-sm z-50 flex items-center justify-center p-6">
            ...
            >
              ...
              <h2 className="text-3xl font-black text-brand-black mb-2">New Counterparty</h2>
              <p className="text-brand-black/40 font-medium mb-8">Add a new legal entity or individual to your database.</p>

              <form onSubmit={handleCreate} className="space-y-6">
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-4">
                    <h4 className="text-xs font-black uppercase tracking-widest text-brand-black/40">Entity Details</h4>
                    <input 
                      type="text" 
                  ...
                  <div className="space-y-4">
                    <h4 className="text-xs font-black uppercase tracking-widest text-brand-black/40">Contact Info</h4>
                    <input 
                      type="email" 
                ...
                <div className="space-y-4">
                  <h4 className="text-xs font-black uppercase tracking-widest text-brand-black/40">Signatory</h4>
                ...
                >
                  {createMutation.isPending ? <Loader2 className="animate-spin" /> : 'Save Counterparty'}
                </button>
              </form>
            </motion.div>
          </div>
