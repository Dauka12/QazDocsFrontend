...
const EmployeesPage = () => {
  ...
  const [inviteData, setInviteData] = useState({ email: '', role: 'EMPLOYEE', organization_id: 1 });
  const [error, setError] = useState<string | null>(null);

  const { data: employees, isLoading } = useQuery({
    queryKey: ['employees'],
    queryFn: () => orgApi.listEmployees(1).then(res => res.data),
  });
  ...
  const inviteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setInviteData({ email: '', role: 'EMPLOYEE', organization_id: 1 });
      setError(null);
    },
    onError: (err: any) => {
      setError(err.response?.data || err.message || 'Failed to invite employee');
    }
  });
  ...
  return (
    <div className="p-12">
      <div className="flex justify-between items-center mb-12">
          <div>
            <h1 className="text-4xl font-black text-brand-black mb-2">Employees</h1>
            <p className="text-brand-black/40 font-bold uppercase tracking-widest text-sm">Manage your team members</p>
          </div>
          <button 
            onClick={() => setIsInviteOpen(true)}
            className="bg-brand-aquamarine text-brand-black px-8 py-4 rounded-2xl font-black flex items-center gap-2 hover:shadow-xl hover:-translate-y-1 transition-all active:scale-95"
          >
            <Plus size={20} />
            Invite Employee
          </button>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-brand-eggshell border-b border-brand-black/5">
                <th className="px-8 py-4 text-xs font-black uppercase tracking-widest text-brand-black/40">Employee</th>
                <th className="px-8 py-4 text-xs font-black uppercase tracking-widest text-brand-black/40">Role</th>
                <th className="px-8 py-4 text-xs font-black uppercase tracking-widest text-brand-black/40">Status</th>
                <th className="px-8 py-4 text-xs font-black uppercase tracking-widest text-brand-black/40">Joined Date</th>
              ...
              {isLoading ? (
                ...
                employees.map((emp: any) => (
                  <tr key={emp.id} className="border-b border-brand-black/5 hover:bg-brand-eggshell/50 transition-colors">
                    ...
                    <td className="px-8 py-6 text-sm font-medium text-brand-black/60">{emp.created_at ? new Date(emp.created_at).toLocaleDateString() : 'N/A'}</td>
                ...
                <tr>
                  <td colSpan={5} className="px-8 py-20 text-center">
                    ...
                    <p className="text-brand-black/40 font-bold">No employees found.</p>
                  </td>
                </tr>
              )}
            </tbody>
        ...
        {isInviteOpen && (
          <div className="fixed inset-0 bg-brand-black/20 backdrop-blur-sm z-50 flex items-center justify-center p-6">
            ...
            >
              ...
              <h2 className="text-3xl font-black text-brand-black mb-2">Invite Employee</h2>
              <p className="text-brand-black/40 font-medium mb-8">Send an invitation to join your organization.</p>
              ...
              <form onSubmit={handleInvite} className="space-y-6">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase tracking-wider text-brand-black/40">Role</label>
                  <select 
